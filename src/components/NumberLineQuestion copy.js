...
const NumberLineQuestion = ({ question, onFinish }) => {
  ...
  const drawNumberLine = () => {
    ...
    // Ticks
    for (let i = 0; i < currentQuestion.sequence.length; i++) {
      const x =
        (width / currentQuestion.sequence.length) * i +
        width / (2 * currentQuestion.sequence.length);
      ctx.beginPath();
  ...
  useEffect(() => {
    ...
    const newInputValues = currentQuestion.sequence
      .filter((n) => n === null)
      .map(() => "");
