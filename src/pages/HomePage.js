...
function HomePage() {
  ...
  // Assuming clsData is nested under jsonData
  const clsData = jsonData?.clsData || []; // Adjust based on your actual structure
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-6 max-w-4xl mx-auto">
      {activeClasses.length > 0 ? (
        activeClasses.map((classItem, index) => (
          <div
            key={classItem._id || index} // Use index as fallback if id doesn't exist
          ...
          >
            <h2 className="text-lg font-semibold text-gray-800 mb-2">
              {classItem._idid === "LKG" || classItem._id === "UKG"
                ? classItem._id
